...
RUN apt update && echo 'Acquire::http::Proxy "http://192.168.2.1:3142";' > /etc/apt/apt.conf.d/00aptproxy && \
    apt update && apt install wget build-essential openssl libssl-dev pkg-config zip unzip sudo -y && \
    cd /tmp/ && unzip OpenSSL_1_1_1j.zip && cd ./openssl-OpenSSL_1_1_1j/ && \
    ...
    cd /squid && tar -xvf squid-5.8.tar.gz && cd squid-5.8 && \
    ./configure --with-default-user=proxy --with-openssl --enable-ssl-crtd --prefix=/squid/ --enable-icap-client --enable-ssl --with-openssl && \
    make && make install && \
    mkdir /squid/ssl_cert && cd /squid/ssl_cert && openssl req -new -newkey rsa:2048 -days 365 -nodes -x509 -extensions v3_ca -keyout squid-self-signed.key -out squid-self-signed.crt \
    ...
    mv /squid/ssl_cert /squid/etc/ssl_cert && \
